        ...
        if (originalResponse.headers("set-cookie").isNotEmpty()) {
            ...
            for (cookie in cookieValue) {
                if (cookie.startsWith("MINI")){
                    MySharedPreferences.setString(DucisLibrary.appContext, com.common.ducis.component.network.HttpConfig.COOKIE_AUTH_ID,cookie,MySharedPreferences.APP_DATA)
                }
                cookieBuffer.append("$cookie;")
            }
            if (cookieBuffer.toString().isNotEmpty()){
                MySharedPreferences.setString(DucisLibrary.appContext, com.common.ducis.component.network.HttpConfig.COOKIE,cookieBuffer.toString(),MySharedPreferences.APP_DATA)
            }
        }
        //用户信息
        originalResponse.header("mrpc-auth-id")?.let {
            MySharedPreferences.setString(DucisLibrary.appContext, com.common.ducis.component.network.HttpConfig.COOKIE_AUTH_ID,it,MySharedPreferences.APP_DATA)
        }
